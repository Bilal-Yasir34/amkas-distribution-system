...
import { Plus, Layers, X, Edit, Trash2 } from 'lucide-react';
...
export function Departments() {
  ...
  const openEdit = (d: Department) => {
    ...
    setName(d.name);
    setCode(d.code);
    setBranch(d.branch_id || 'Head Office');
  ...
  const handleDelete = (id: string, dName: string) => {
    if (confirm(`Are you sure you want to delete department ${dName}?`)) {
      deleteDepartment(id);
      toast.success(`Department ${dName} deleted`);
    }
  };
                    ...
                    <td className="px-4 py-3">
                      <button
                        ...
                        className={`rounded-md px-2 py-0.5 text-[10px] font-semibold ${
                          d.is_active ? 'bg-emerald-500/10 text-emerald-500' : 'bg-slate-200 text-slate-500'
                        }`}
                      >
